...
function peakflops(len=1024*1024*100; type=Float32)
    a = MtlArray(rand(type, len))
    b = MtlArray(rand(type, len))
    c = MtlArray(rand(type, len))
    out = similar(a)
    ...
    flopcount = (99*3+1) * 2.0 * len
    secs = minimum(bench.times) * 1e-9
    flops = flopcount / secs

    println("TFlops: $(round(flops/1e12; digits=2)), len=$len, secs=$secs, flopcount=$flopcount")
    return bench
end
